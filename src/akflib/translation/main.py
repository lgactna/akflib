...
from akflib.translation.util import align_text
...
def execution_entrypoint(
    scenario: AKFScenario, modules: dict[str, AKFModule[Any, Any]], machine_name: str
) -> None:
    """
    Entrypoint for the declarative translator.
    """
    # Global state variables
    state: dict[str, Any] = {"machine_name": machine_name}
...
@click.option(
    # TODO: The notion of a machine name is strongly bound to VirtualBox, but
    # *is* a notion enforced by the hypervisor ABC. The larger issue is allowing
    # users to use different hypervisor backends, though I don't think we'll
    # run into that in the near future.
    "--machine-name",
    type=str,
    help="The virtual machine to use when running the scenario.",
)
@click.option(
    "--output-file",
...
def main(
    ...
    if execute:
        ...
        # Assert that the machine name is set
